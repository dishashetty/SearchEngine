...
public class DocWordCount extends Configured implements Tool {
	...
	public static void main( String[] args) throws  Exception {
		int res  = ToolRunner .run( new DocWordCount(), args); //initializing the run function after getting instance of the class
	...
	public int run( String[] args) throws  Exception {
		...
		job.setOutputValueClass( IntWritable .class); // value represents number of times that word appears(key-value pair)
	...
	public static class Map extends Mapper<LongWritable ,  Text ,  Text ,  IntWritable > {
		private final static IntWritable one  = new IntWritable( 1); // defining global variable
		...
		public void map( LongWritable offset,  Text lineText,  Context context) //invokes map method once for each key value pair from your input source
			...
			String line  = lineText.toString(); //converting text object to string
	...
	public static class Reduce extends Reducer<Text ,  IntWritable ,  Text ,  IntWritable > {
		@Override 
		public void reduce( Text word,  Iterable<IntWritable > counts,  Context context) //reducer function to give overall count of word from all mappers
				throws IOException,  InterruptedException {
			int sum  = 0;
			for ( IntWritable count  : counts) {
				sum  += count.get();
			}
			context.write(word,  new IntWritable(sum)); // writing result to reducer context
		}
	}
}
